...
一个**纯前端、零依赖**的二维平面结构分析工具，运行在浏览器中。采用**直接刚度法**对**框架 / 平面 / 壳**单元进行线弹性分析，可生成位移云图、弯矩/剪力/轴力图、面应力云图（von Mises、σx、σy、τxy、主应力、膜内力），并支持在任意截面读取内力和在每个面单元读取应力（SAP2000 风格的定位点）。内置**模型保存/打开**、**亮/暗主题**与**单位切换**（m/mm、kN/N）。
...
6. 需要时在右上角「保存」导出模型 JSON，或「打开」恢复；底部状态栏可切换 **长度 / 力** 单位，右上角可切换 **亮 / 暗主题**。
...
**单位约定**：内部计算以米 / 千牛为基本单位（长度 m、力 kN、弯矩 kN·m、面荷载 kN/m²、应力 kPa(=kN/m²)、刚度 EA(kN)、EI(kN·m²)、GA(kN)）；界面可通过底部状态栏切换为 **mm / N**，仅影响显示与输入，内部换算自动完成（应变以米计，应力按当前力单位显示为 kPa 或 Pa）。荷载“垂直杆件、向下为正”。
